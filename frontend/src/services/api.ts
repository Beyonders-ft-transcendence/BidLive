...
import { User, Auction, Bid, Stream, AuctionStatus, Message, PrivateChatMessage, PrivateConversation } from '../types';
...
function extractImages(item: any): string[] {
  ...
  const urls = item.images
    .map((img: any) => {
      ...
      return img.image_url || img.file?.url || img.url || null;
    })
    .filter(Boolean);
...
function mapChatUser(apiUser: any): User {
  return {
    id: String(apiUser?.id || ''),
    name: apiUser?.full_name || apiUser?.name || apiUser?.username || 'Usuario',
    email: apiUser?.email || '',
    avatar:
      apiUser?.avatar_url ||
      apiUser?.avatar ||
      `https://api.dicebear.com/7.x/identicon/svg?seed=${apiUser?.username || apiUser?.id || 'chat-user'}`,
    role: mapBackendRole(apiUser?.roles),
    balance: Number(apiUser?.balance || 0),
    status: 'ACTIVE',
    bio: apiUser?.bio || 'Membro do portal BidLive.',
    permissions: mapUserPermissions(apiUser || {}),
  };
}

function mapRoomMessage(apiMessage: any): Message {
  const sender = apiMessage.sender || {};
  const name = sender.full_name || sender.name || sender.username || 'Usuario';
  return {
    id: String(apiMessage.id),
    roomId: String(apiMessage.room || apiMessage.room_id || ''),
    senderName: name,
    senderAvatar:
      sender.avatar_url ||
      sender.avatar ||
      `https://api.dicebear.com/7.x/identicon/svg?seed=${sender.username || sender.id || name}`,
    content: apiMessage.message || apiMessage.content || '',
    timestamp: apiMessage.created_at || apiMessage.timestamp || new Date().toISOString(),
    role: mapBackendRole(sender.roles),
  };
}

function mapPrivateMessage(apiMessage: any): PrivateChatMessage {
  const sender = apiMessage.sender || {};
  const name = sender.full_name || sender.name || sender.username || 'Usuario';
  return {
    id: String(apiMessage.id),
    conversationId: String(apiMessage.conversation || apiMessage.conversation_id || ''),
    senderId: String(sender.id || apiMessage.sender_id || ''),
    senderName: name,
    senderAvatar:
      sender.avatar_url ||
      sender.avatar ||
      `https://api.dicebear.com/7.x/identicon/svg?seed=${sender.username || sender.id || name}`,
    content: apiMessage.message || apiMessage.content || '',
    isRead: Boolean(apiMessage.is_read),
    timestamp: apiMessage.created_at || apiMessage.timestamp || new Date().toISOString(),
  };
}

function mapPrivateConversation(apiConversation: any, currentUserId: string): PrivateConversation {
  const userOne = apiConversation.user_one || {};
  const userTwo = apiConversation.user_two || {};
  const participant = String(userOne.id) === String(currentUserId) ? userTwo : userOne;
  return {
    id: String(apiConversation.id),
    participant: mapChatUser(participant),
    lastMessage: apiConversation.last_message?.message || null,
    unreadCount: Number(apiConversation.unread_count || 0),
    timestamp:
      apiConversation.last_message?.created_at ||
      apiConversation.updated_at ||
      apiConversation.created_at ||
...
export const apiService = {
    ...
    try {
      if (accessToken) {
        ...
        if (!response.ok) {
          ...
          return {
            success: false,
            message: payload?.message || payload?.detail || 'Logout remoto falhou.',
          };
        }
      }
      ...
      return {
        success: false,
        message: error?.message || 'Nao foi possivel revogar a sessao no backend.',
      };
    } finally {
      clearStoredAuth();
    }
  ...
  async getAuctionChatRoom(auctionId: string): Promise<{ success: boolean; room?: any; message?: string }> {
    const res = await request<any>(`/chat/auctions/${auctionId}/room/`);
    if (res.success && res.data) {
      return { success: true, room: res.data };
    }
    return { success: false, message: res.message || 'Erro ao carregar sala de chat.' };
  },

  async getAuctionChatMessages(auctionId: string): Promise<{ success: boolean; messages?: Message[]; message?: string }> {
    const res = await request<any>(`/chat/auctions/${auctionId}/messages/`);
    if (res.success && res.data) {
      return { success: true, messages: extractList(res.data).map(mapRoomMessage) };
    }
    return { success: false, messages: [], message: res.message || 'Erro ao carregar mensagens.' };
  },

  async sendAuctionChatMessage(auctionId: string, message: string): Promise<{ success: boolean; message?: string; chatMessage?: Message }> {
    const res = await request<any>(`/chat/auctions/${auctionId}/send/`, {
      method: 'POST',
      body: JSON.stringify({ message }),
    });
    if (res.success && res.data) {
      return { success: true, message: res.message || 'Mensagem enviada.', chatMessage: mapRoomMessage(res.data) };
    }
    return { success: false, message: res.message || 'Erro ao enviar mensagem.' };
  },

  async deleteAuctionChatMessage(messageId: string): Promise<{ success: boolean; message?: string }> {
    const res = await request(`/chat/auctions/messages/${messageId}/`, { method: 'DELETE' });
    return { success: res.success, message: res.message };
  },

  async getPrivateConversations(currentUserId: string): Promise<{ success: boolean; conversations?: PrivateConversation[]; message?: string }> {
    const res = await request<any>('/chat/conversations/');
    if (res.success && res.data) {
      return {
        success: true,
        conversations: extractList(res.data).map((conversation: any) => mapPrivateConversation(conversation, currentUserId)),
      };
    }
    return { success: false, conversations: [], message: res.message || 'Erro ao carregar conversas.' };
  },

  async getPrivateMessages(conversationId: string): Promise<{ success: boolean; messages?: PrivateChatMessage[]; message?: string }> {
    const res = await request<any>(`/chat/conversations/${conversationId}/messages/`);
    if (res.success && res.data) {
      return { success: true, messages: extractList(res.data).map(mapPrivateMessage) };
    }
    return { success: false, messages: [], message: res.message || 'Erro ao carregar mensagens privadas.' };
  },

  async sendPrivateMessage(recipientId: string, message: string): Promise<{ success: boolean; message?: string; chatMessage?: PrivateChatMessage }> {
    const res = await request<any>('/chat/conversations/send/', {
      method: 'POST',
      body: JSON.stringify({ recipient_id: Number(recipientId), message }),
    });
    if (res.success && res.data) {
      return { success: true, message: res.message || 'Mensagem enviada.', chatMessage: mapPrivateMessage(res.data) };
    }
    return { success: false, message: res.message || 'Erro ao enviar mensagem privada.' };
  },

  async markPrivateConversationRead(conversationId: string): Promise<{ success: boolean; message?: string }> {
    const res = await request(`/chat/conversations/${conversationId}/read/`, { method: 'POST' });
    return { success: res.success, message: res.message };
  },

  async deletePrivateMessage(messageId: string): Promise<{ success: boolean; message?: string }> {
    const res = await request(`/chat/conversations/messages/${messageId}/`, { method: 'DELETE' });
    return { success: res.success, message: res.message };
  },

  async sendAnalyticsEvent(eventType: string, metadata: Record<string, any> = {}): Promise<{ success: boolean; message?: string }> {
    const res = await request('/analytics/events/', {
      method: 'POST',
  ...
  async getAnalyticsStats(): Promise<{ success: boolean; data?: any; message?: string }> {
    const res = await request<any>('/analytics/stats/');
    if (res.success && res.data) {
      return { success: true, data: res.data };
    }
    return { success: false, message: res.message || 'Erro ao carregar estatísticas.' };
  },
  ...
  async getUsers(): Promise<{ success: boolean; users?: User[]; message?: string }> {
    ...
    if (res.success && res.data) {
      const list = extractList(res.data);
      return { success: true, users: list.map((u: any) => mapUser(u)) };
    }
    return { success: false, message: res.message || 'Erro ao carregar usuários.' };
  },

  async banUser(userId: string, status: 'ACTIVE' | 'BANNED'): Promise<{ success: boolean; message?: string; user?: User }> {
    const res = await request<any>(`/users/${userId}/ban/`, {
      method: 'POST',
      body: JSON.stringify({ status }),
    });
    if (res.success && res.data) {
      return { success: true, message: res.message, user: mapUser(res.data) };
    }
    return { success: false, message: res.message || 'Erro ao alterar status do usuário.' };
  },
};
