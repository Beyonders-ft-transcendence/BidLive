...
export default function App() {
  ...
  const placeBid = async (auctionId: string, amount: number) => {
    ...
    if (res.success) {
      apiService.sendAnalyticsEvent('bid.placed', { auction_id: auctionId, amount });
  ...
  const buyNowSubmit = async (auctionId: string) => {
    ...
    if (res.success) {
      apiService.sendAnalyticsEvent('buynow.clicked', { auction_id: auctionId });
  ...
  const toggleWatchlist = async (id: string, e: React.MouseEvent) => {
    ...
    apiService.sendAnalyticsEvent('watchlist.toggle', { auction_id: id, is_added: !isCurrentlyWatching });
  ...
  const navigateToDetails = (auctionId: string) => {
    ...
    apiService.sendAnalyticsEvent('auction.page_view', { auction_id: auctionId });
  };
