...
export default function AuctionDetailPage() {
    ...
    const renderBidsHistory = () => (
        <>
            <div className="px-6 py-3 border-b border-slate-800/60 flex items-center gap-2 bg-slate-800/20">
                <Gavel size={14} className="text-slate-400" />
                <span className="text-xs font-bold uppercase tracking-wider text-slate-300">
                    Histórico de Lances ({auction.bids_count || bids.length})
                </span>
            </div>
            <div className="divide-y divide-slate-800/60 max-h-[300px] overflow-y-auto bg-slate-800/20">
                {bids.length > 0 ? (
                    bids.map((bid, i) => (
        ...
        <div className="min-h-screen bg-[#0A0F1C] font-sans antialiased flex flex-col">
            ...
            <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-10">
                ...
                <div className="flex flex-col lg:hidden gap-6">
                    ...
                    <div className="bg-[#111827] border border-slate-800 rounded-sm shadow-md shadow-black/20 overflow-hidden">
                        {renderBidding()}
                    </div>
                    
                    {showChat && (
                        <div className="bg-[#111827] border border-slate-800 rounded-sm shadow-md shadow-black/20 flex flex-col h-[400px]">
                            {renderChat()}
                        </div>
                    )}
