...
export default function AdminPanel({
  ...
  const toggleUserBan = async (userId: string) => {
    ...
    if (res.success) {
      ...
      setShowToast(`Erro ao alterar status: ${res.message}`);
      setTimeout(() => setShowToast(null), 3000);
    }
  };
  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      <div className="bg-zinc-950 p-6 rounded-xl border border-zinc-900 shadow-xl min-h-[300px]">
        ...
        {/* TAB 3: server telemetry & analytics stats */}
        {activeTab === 'live-telemetry' && (
          <div className="space-y-6">
            {stats && (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-zinc-900/30 border border-zinc-900 p-4 rounded-xl space-y-2 text-left">
                  <span className="block text-[10px] text-zinc-500 font-mono uppercase font-semibold">Usuários Ativos (Analytics)</span>
                  <div className="flex items-baseline gap-2">
                    <span className="text-2xl font-mono font-bold text-sky-400">{stats.active_users?.online_now ?? 0}</span>
                    <span className="text-xs text-zinc-400">online</span>
                  </div>
                  <span className="block text-[10px] text-zinc-500 font-sans">
                    {stats.active_users?.active_last_hour ?? 0} ativos na última hora
                  </span>
                </div>

                <div className="bg-zinc-900/30 border border-zinc-900 p-4 rounded-xl space-y-2 text-left">
                  <span className="block text-[10px] text-zinc-500 font-mono uppercase font-semibold">Conversão de Leilões</span>
                  <div className="flex items-baseline gap-2">
                    <span className="text-2xl font-mono font-bold text-emerald-400">
                      {stats.conversion_metrics?.auction_conversion_rate_percentage ?? 0}%
                    </span>
                    <span className="text-xs text-zinc-400">arrematados</span>
                  </div>
                  <span className="block text-[10px] text-zinc-500 font-sans">
                    {stats.conversion_metrics?.sold_auctions ?? 0} vendidos de {stats.conversion_metrics?.total_ended_auctions ?? 0} encerrados
                  </span>
                </div>

                <div className="bg-zinc-900/30 border border-zinc-900 p-4 rounded-xl space-y-2 text-left">
                  <span className="block text-[10px] text-zinc-500 font-mono uppercase font-semibold">Engajamento de Lances</span>
                  <div className="flex items-baseline gap-2">
                    <span className="text-2xl font-mono font-bold text-indigo-400">
                      {stats.conversion_metrics?.bidder_engagement_rate_percentage ?? 0}%
                    </span>
                    <span className="text-xs text-zinc-400">participação</span>
                  </div>
                  <span className="block text-[10px] text-zinc-500 font-sans">
                    {stats.conversion_metrics?.unique_bidders ?? 0} licitantes de {stats.conversion_metrics?.total_active_users ?? 0} usuários ativos
                  </span>
                </div>
              </div>
            )}
            ...
            {stats?.top_event_types?.length > 0 && (
              <div className="bg-zinc-900/10 border border-zinc-900 p-4 rounded-xl text-left space-y-3">
                <span className="block text-[10px] text-zinc-500 font-mono uppercase font-semibold">Eventos Mais Frequentes</span>
                <div className="flex flex-wrap gap-2">
                  {stats.top_event_types.map((evt: any, idx: number) => (
                    <div key={idx} className="bg-zinc-900 border border-zinc-800 px-3 py-1.5 rounded-lg text-xs flex items-center gap-2 font-mono">
                      <span className="text-zinc-400">{evt.event_type}</span>
                      <span className="bg-sky-950 text-sky-400 px-1.5 py-0.5 rounded text-[10px] font-bold">{evt.count}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl bg-zinc-950 border border-zinc-900 font-mono text-[11px] text-left leading-relaxed text-zinc-400 space-y-2 h-72 overflow-y-auto scrollbar-thin">
                {systemLogs.map(log => (
                  <div key={log.id} className="flex gap-2.5 items-start">
                    ...
                    <span className="text-zinc-650 font-bold">[{log.module}]</span>
                    <div className="flex-1">
                      <span className="text-zinc-300">{log.message}</span>
                      <span className="block text-[9px] text-zinc-650 mt-0.5 italic">{log.details}</span>
                    </div>

                    <span className="text-zinc-650 font-semibold">{new Date(log.timestamp).toLocaleTimeString('pt-BR')}</span>
                  </div>
                ))}
              </div>
            </div>
