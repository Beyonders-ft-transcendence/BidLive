...
import { X, Gavel, Shield, Ban, ShieldCheck, Loader2 } from "lucide-react";
import { useTranslation } from "react-i18next";
import { toast } from "sonner";
import Avatar from "@/components/common/Avatar";
import socialService from "@/services/social.service";
import { useBlockUserMutation, useUnblockUserMutation } from "@/hooks/useSocial";
...
export default function PublicProfileModal({
    ...
    // O backend não expõe o estado de bloqueio; começamos como "não bloqueado"
    // e corrigimos se a API responder que já estava (ver handleBlock).
    // Estado inicial vem da memória local de bloqueios (ver blocked.store.ts);
    // a resposta "já está bloqueado" da API continua a corrigir o que faltar.
    ...
    const [isBlocked, setIsBlocked] = useState(storeSaysBlocked);
    const [confirmingBlock, setConfirmingBlock] = useState(false);
    // Confirmação também no desbloqueio: sem ela, o botão que aparece no mesmo
    // sítio após bloquear com sucesso desfazia o bloqueio num clique acidental.
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="bg-card border border-border w-full max-w-sm rounded-xl shadow-2xl relative z-10 animate-in zoom-in-95 duration-200 overflow-hidden flex flex-col">
                ...
                <div className="pt-14 pb-6 px-6 text-center">
                    ...
                    {/* Bloquear / desbloquear (ISSUE FE-010) — nunca para o próprio perfil */}
                    {!isSelf && (
