...
import { Room, createLocalVideoTrack, createLocalAudioTrack, LocalVideoTrack, LocalAudioTrack } from 'livekit-client';
...
export default function LiveStreamBroadcaster({ streamId, auctionId }: LiveStreamBroadcasterProps) {
  ...
  const startBroadcast = async () => {
    try {
      ...
      const tokenRes = await apiService.getLiveKitToken(auctionId, streamId, 'broadcaster', 'Streamer Principal');
      if (!tokenRes.success || !tokenRes.token) {
        throw new Error('Falha ao obter token do backend: ' + (tokenRes.message || 'Sem motivo'));
      }

      const roomUrl = tokenRes.url || import.meta.env.VITE_LIVEKIT_URL || 'ws://localhost:7880';
      console.log('Conectando ao LiveKit:', roomUrl, 'Token:', tokenRes.token);
      ...
      try {
        ...
        console.log('Conectado à sala LiveKit com sucesso!');
      } catch (e: any) {
      ...
      try {
        // Sem restrições de 'facingMode' ou resolução, para evitar "device not found" em PCs sem webcam específica.
        videoTrack = await createLocalVideoTrack().catch(e => {
          console.warn("Câmera não encontrada ou em uso por outro app:", e);
          return null; // Retorna null se não houver câmera, mas permite continuar
        });
        ...
        if (!videoTrack && !audioTrack) {
          throw new Error('Nenhuma câmera ou microfone físico encontrado conectado ao seu computador. Conecte uma webcam/microfone e tente novamente.');
        }
      } catch (e: any) {
      ...
      try {
        ...
        throw new Error('Erro ao fazer upload (publish) da mídia para o LiveKit: ' + e.message);
      }
  ...
  return (
    <div className="bg-zinc-950 border border-zinc-800 p-5 rounded-xl shadow-lg mt-6">
      ...
      <div className="relative aspect-video bg-zinc-900 rounded-lg overflow-hidden mb-4 border border-zinc-800">
        <video 
          ref={videoRef} 
          className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-300 ${isPublishing && !isVideoMuted ? 'opacity-100' : 'opacity-0'}`} 
          muted // Muted para o streamer não ouvir o próprio eco
          autoPlay
          playsInline
        />
        ...
        {!isPublishing ? (
          ...
          <div className="flex w-full items-center justify-between gap-4">
            <div className="flex gap-2">
              <button 
                onClick={toggleMute}
                className={`p-2.5 rounded-lg border transition-colors ${isMuted ? 'bg-red-500/10 border-red-500/50 text-red-400' : 'bg-zinc-800 border-zinc-700 text-zinc-300 hover:bg-zinc-700'}`}
              ...
              <button 
                onClick={toggleVideo}
                className={`p-2.5 rounded-lg border transition-colors ${isVideoMuted ? 'bg-red-500/10 border-red-500/50 text-red-400' : 'bg-zinc-800 border-zinc-700 text-zinc-300 hover:bg-zinc-700'}`}
