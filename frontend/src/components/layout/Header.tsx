            ...
            {isMobileMenuOpen && (
                <div className="md:hidden border-t border-border bg-background px-4 py-4 space-y-4 shadow-lg">
                    ...
                    <nav className="flex flex-col gap-3 font-medium text-foreground">
                        <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-primary py-2 border-b border-border/50">Início</Link>
                        <Link to="/leiloes" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-primary py-2 border-b border-border/50">Leilões</Link>
                        <Link to="/favoritos" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-primary py-2 border-b border-border/50 flex items-center gap-2">
                            <Heart size={18} /> Favoritos
                        </Link>
                        
                        {/* Mobile Language Selection */}
                        <div className="py-2 border-b border-border/50">
                            <p className="text-muted-foreground text-sm mb-2 flex items-center gap-2"><Globe size={16} /> Idioma</p>
                            <div className="flex gap-2">
                                <button onClick={() => { setLanguage("PT"); setIsMobileMenuOpen(false); }} className={`px-3 py-1 text-sm rounded border ${language === "PT" ? "bg-primary text-primary-foreground border-primary" : "border-border text-foreground"}`}>PT</button>
                                <button onClick={() => { setLanguage("EN"); setIsMobileMenuOpen(false); }} className={`px-3 py-1 text-sm rounded border ${language === "EN" ? "bg-primary text-primary-foreground border-primary" : "border-border text-foreground"}`}>EN</button>
                                <button onClick={() => { setLanguage("AR"); setIsMobileMenuOpen(false); }} className={`px-3 py-1 text-sm rounded border ${language === "AR" ? "bg-primary text-primary-foreground border-primary" : "border-border text-foreground"}`}>AR</button>
                            </div>
                        </div>
                    </nav>
