        ...
        <div className="flex items-center gap-4 md:gap-8 h-full">
          ...
          <ul className="hidden lg:flex items-center h-full">
            {menuItems.map((item) => {
              ...
              return (
                <li key={item.label} className={`flex items-center h-full px-4 md:px-5 text-xs md:text-sm font-semibold transition-all cursor-pointer ${isActive ? 'border-b-[3px] border-primary text-white bg-gray-800/50' : 'text-gray-300 hover:text-white hover:bg-gray-800'}`}>
