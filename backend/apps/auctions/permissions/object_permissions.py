...
class IsLiveStreamOwnerOrManager(BasePermission):
    def has_object_permission(self, request, view, obj):
        if request.method in SAFE_METHODS:
            return True
        auction = getattr(obj, "auction", None)
        if auction and getattr(auction, "item", None) and auction.item.seller_id == request.user.id:
            return True
        if getattr(obj, "streamer_id", None) == request.user.id:
            return True
        if user_has_permission(user=request.user, permission_name="auction.manage"):
            return True
        return bool(request.user and request.user.has_role("admin"))
