...
if [ -f /run/secrets/livekit_credenciais ] && [ -f /run/secrets/redis_credenciais ]; then
    ...
    # CORRIGIDO: O LiveKit não usa o formato de URI padrão do Redis aqui.
    # Guardamos apenas o endereço "host:porta" e a senha separada.
...
# Mudado para /tmp para garantir permissão de escrita total do usuário livekit
