...
def test_seed_demo_data_creates_users_and_auctions():
    ...
    assert User.objects.filter(email__iendswith=f"@{DEMO_EMAIL_DOMAIN}").count() == 4
    assert AuctionItem.objects.filter(seller__email__iendswith=f"@{DEMO_EMAIL_DOMAIN}").count() == 5
    auction_qs = Auction.objects.filter(item__seller__email__iendswith=f"@{DEMO_EMAIL_DOMAIN}")
    assert auction_qs.count() == 5
    assert LiveStream.objects.filter(auction__in=auction_qs, is_live=True).count() == 2
    assert Message.objects.filter(room__auction__in=auction_qs).count() == 3
...
def test_seed_management_command_is_idempotent():
    ...
    assert users_after_first == users_after_second == 4
